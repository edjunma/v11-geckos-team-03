...
class List extends React.Component {
	...
	setSpansUpdate = addCardHeight => {
		...
		const height =
			this.headingRef.current.clientHeight + this.cardsRef.current.clientHeight + addCardHeight;
	...
	setSpansUpdateForCard = addCardHeight => {
		...
		const height = this.headingRef.current.clientHeight + cardsHeight + addCardHeight;
	...
	cardsArray = () =>
		this.state.cards.map(card => {
			return <CardModal key={card.title} cardTitle={card.title} />;
		});

	render() {
		return (
			<div className='list' ref={this.listRef} style={{ gridRowEnd: `span ${this.state.spans}` }}>
